...
export function InvitationCopyPage() {
  ...
  const { mutate: updateInvitation } = useMutation({
    ...
    onSuccess: () => {
      queryClient
        .invalidateQueries({ queryKey: ["invitations"] })
        .then(() => navigate("/invitations"));
    },
  });
  ...
  const form = useFormik<UpdateInvitationPayloadType>({
    ...
    onSubmit: (values) =>
      updateInvitation(values as CreateInvitationPayloadType),
